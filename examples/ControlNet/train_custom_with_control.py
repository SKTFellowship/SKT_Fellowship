...
def collate_fn(examples, with_prior_preservation):
    ...
    condition_values = [example["instance_condition_images"] for example in examples]
    ...
    if with_prior_preservation:
        ...
        condition_values += [example["class_condition_images"] for example in examples]
        mask += [example["class_mask"] for example in examples]

    input_ids = torch.cat(input_ids, dim=0)
    pixel_values = torch.stack(pixel_values)
    condition_values = torch.stack(condition_values)
    ...
    batch = {"input_ids": input_ids, "pixel_values": pixel_values,
             "condition_values": condition_values, "mask": mask.unsqueeze(1)}
    return batch
...
class CustomControlDataset(Dataset):
    """
    ...
    def __init__(
        ...
        for concept in concepts_list:
            ...
            for x in Path(concept["instance_data_dir"]).iterdir():
                if x.is_file():
                    condition_name = os.path.basename(x)
                    condition_path = os.path.join(concept["instance_condition_dir"], condition_name)
                    inst_img_path.append((x, concept["instance_prompt"], condition_path))
            ...
            if with_prior_preservation:
                ...
                conditions_path = []
                for x in class_images_path:
                    condition_name = os.path.basename(x)
                    condition_path = os.path.join(concept["class_condition_dir"], condition_name)
                    conditions_path.append(condition_path)
                class_img_path = [(x, y, z) for (x, y, z) in zip(class_images_path, class_prompt, conditions_path)]
    ...
    def preprocess(self, image, condition, scale, resample):
        ...
        condition = condition.resize((scale, scale), resample=resample)
        condition = np.array(condition).astype(np.uint8)

        condition_image = np.zeros((self.size, self.size, 3), dtype=np.float32)
        instance_image = np.zeros((self.size, self.size, 3), dtype=np.float32)
        mask = np.zeros((self.size // factor, self.size // factor))
        if scale > self.size:
            instance_image = image[top : top + inner, left : left + inner, :]
            condition_image = condition[top : top + inner, left : left + inner, :]
            mask = np.ones((self.size // factor, self.size // factor))
        else:
            instance_image[top : top + inner, left : left + inner, :] = image
            condition_image[top: top + inner, left: left + inner, :] = condition
            mask[
                top // factor + 1 : (top + scale) // factor - 1, left // factor + 1 : (left + scale) // factor - 1
            ] = 1.0
        return instance_image, condition_image, mask

    def __getitem__(self, index):
        example = {}
        instance_image, instance_prompt, condition_path = self.instance_images_path[index % self.num_instance_images]
        instance_image = Image.open(instance_image)
        condition_image = cv2.imread(condition_path)
        ...
        #     )
        instance_image, condition_image, mask = self.preprocess(instance_image,
                                                                condition_image, random_scale, self.interpolation)
        ...
        example["instance_condition_images"] = torch.from_numpy(condition_image).permute(2, 0, 1)
        ...
        if self.with_prior_preservation:
            class_image, class_prompt, condition_path = self.class_images_path[index % self.num_class_images]
            ...
            example["class_condition_images"] = self.image_transforms(condition_image)
...
def main(args):
    ...
    train_dataset = CustomControlDataset(
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet), accelerator.accumulate(text_encoder):
                ...
                # with condition
                if with_condition > 0.5:
                    controlnet_image = batch["condition_values"].to(dtype=weight_dtype)
